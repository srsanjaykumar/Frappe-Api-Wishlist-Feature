...
# @frappe.whitelist(allow_guest=1)
# def calc(question_id,question_values):
# 	cat_doc_name=[]
# 	Food=[] 
# 	Transport=[] 
# 	Energy=[] 
...
# 	for ids in range(len(question_id)):
		
# 		doc_name=frappe.get_list("ALLQuestions",filters={'uid':question_id[ids]})
# 		docs= frappe.get_doc("ALLQuestions",doc_name)
# 		category=docs.category
# 		pair[docs.name]=question_values[ids]
# 		print(question_id[ids] , doc_name ,category)
# 		if category == "Food":
# 			Food.append(doc_name)
# 		if category == "Transport":
# 			Transport.append(doc_name)
# 		if category == "Energy":
# 			Energy.append(doc_name)
# 		if category == "Clothing":
# 			Clothing.append(doc_name)

# 	foodsum=0
# 	for doc in Food:
# 		docs= frappe.get_doc("ALLQuestions",doc)
# 		value=0
# 		for k in range(len(docs.options)):
# 			if(docs.options[k].proc_name == pair[docs.name]):
# 				option1=docs.options[k].proc_option
# 				if len(option1)== 0:
# 					continue
# 				value=float(period_dict['Food'])*float(option1)
# 				footprint=frappe.get_doc("Footprint1",docs.footprint).carbon_foot
# 				value=value*float(footprint)
...
# 				value=float(period_dict['Energy'])*float(option1)
# 				footprint=frappe.get_doc("Footprint1",docs.footprint).carbon_foot
# 				value=value*float(footprint)
...
# 					continue
# 				footprint=frappe.get_doc("Footprint1",docs.footprint).carbon_foot
# 				value=float(option1)*float(footprint)
...
# 	return period_dict
	# return Food,Transport,Energy,Clothing
